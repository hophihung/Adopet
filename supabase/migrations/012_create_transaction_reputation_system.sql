-- =====================================================
-- CREATE TRANSACTION AND REPUTATION SYSTEM
-- Hệ thống giao dịch và điểm uy tín cho người bán
-- =====================================================

-- 1. Add reputation_points and avatar_frame to profiles table
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS reputation_points integer DEFAULT 0,
ADD COLUMN IF NOT EXISTS avatar_frame text DEFAULT 'default' CHECK (avatar_frame IN ('default', 'bronze', 'silver', 'gold', 'platinum', 'diamond'));
...
-- 3. Add transaction_code to messages table (optional, for linking messages to transactions)
...
-- 7. Function to generate unique transaction code
...
-- 8. Function to update reputation when transaction is completed
...
BEGIN
  ...
  IF NEW.status = 'completed' AND (OLD.status IS NULL OR OLD.status != 'completed') THEN
    ...
    -- Formula: 1 point per 100,000 VND (or 1 point per 10 units if amount is in smaller currency)
    -- You can adjust this formula based on your needs
    total_points := COALESCE(OLD.reputation_points, 0) + GREATEST(1, FLOOR(NEW.amount / 100000));
...
-- 9. Trigger to update reputation when transaction is completed
...
-- 10. Function to update updated_at for transactions
...
-- 12. Function to confirm transaction completion (for buyer)
...
-- 13. Function to get seller's reputation tier
