-- =====================================================
-- REQUIRE ADMIN APPROVAL FOR REELS
-- =====================================================
-- 1. Add audit columns (if needed)
...
-- 2. Ensure moderation only updates metadata (no auto-approve)
...
DECLARE
  reel_record RECORD;
BEGIN
  SELECT media_type, video_url, image_url, thumbnail_url INTO reel_record
  FROM public.reels
  ...
  -- Reject immediately if sensitive or not pet-related
  IF is_sensitive_param = TRUE THEN
    UPDATE public.reels
    SET 
      is_sensitive = TRUE,
      is_pet_related = is_pet_related_param,
      moderation_reason = COALESCE(moderation_reason_param, 'Nội dung nhạy cảm'),
      status = 'rejected',
      rejected_by = NULL,
      rejected_at = NOW(),
      updated_at = NOW()
  ...
  IF is_pet_related_param = FALSE THEN
    ...
    SET 
      ...
      moderation_reason = COALESCE(moderation_reason_param, 'Không liên quan đến thú cưng'),
      status = 'rejected',
      rejected_by = NULL,
      rejected_at = NOW(),
      updated_at = NOW()
  ...
  -- Otherwise keep pending for admin approval
  UPDATE public.reels
  SET 
    is_sensitive = is_sensitive_param,
    is_pet_related = is_pet_related_param,
...
-- 3. Track admin who approves/rejects
...
-- 4. Allow admins to view pending reels
...
-- 5. Optional view for pending reels
