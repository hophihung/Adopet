...
import { BankAccountService, UpdateBankAccountInput, SellerBankAccount } from '@/src/features/payout/services/bankAccount.service';
import { colors } from '@/src/theme/colors';

const VIETNAM_BANKS = [
  'Vietcombank', 'BIDV', 'VietinBank', 'Agribank', 'ACB', 'Techcombank',
  'MBBank', 'VPBank', 'TPBank', 'Sacombank', 'HDBank', 'SHB', 'VIB',
  'Eximbank', 'MSB', 'OCB', 'SeABank', 'PVcomBank', 'VietABank',
  'BacABank', 'NamABank', 'PGBank', 'ABBank', 'NCB', 'OceanBank',
  'GPBank', 'LienVietPostBank', 'KienLongBank', 'DongABank', 'PublicBank',
  'SCB', 'VCCB', 'BAOVIET Bank', 'VietBank', 'CBBank', 'OCEANBANK',
  'PGBank', 'PVcomBank',
];
...
export default function EditBankAccountScreen() {
  ...
  const [formData, setFormData] = useState<UpdateBankAccountInput>({
    id: id!,
    bank_name: '',
    account_number: '',
    account_holder_name: '',
  ...
  useEffect(() => {
    if (!isSeller || !user?.id || !id) {
      Alert.alert('Lỗi', 'Không hợp lệ');
      router.back();
      return;
    }
    loadBankAccount();
  ...
  const loadBankAccount = async () => {
    ...
    try {
      setLoading(true);
      const accounts = await BankAccountService.getBySeller(user.id);
      const account = accounts.find(a => a.id === id);
      
      if (!account) {
        Alert.alert('Lỗi', 'Không tìm thấy tài khoản ngân hàng', [
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSaving(true);
      await BankAccountService.update(formData, user.id);
      Alert.alert('Thành công', 'Đã cập nhật tài khoản ngân hàng thành công', [
        { text: 'OK', onPress: () => router.back() },
      ]);
    } catch (error: any) {
      console.error('Error updating bank account:', error);
      Alert.alert('Lỗi', error.message || 'Không thể cập nhật tài khoản ngân hàng');
    } finally {
      setSaving(false);
    }
  };
